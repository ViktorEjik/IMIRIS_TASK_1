...
class Server{
  ...
  public:
    ...
    int set_task(){
      if (this->cpu_state == IDLE)
      {
        this->cpu_state = RUN;
        return 3 + this->ID;
      }
      else throw "CPU is buise!";
    }
...
int main(int argc, char **argv )
{
  ...
  int cpu_state = IDLE;
  float run_begin; // 
  Server server_0 = Server(0);
  Server server_1 = Server(1);
  srand(2019);
  ...
  while((curr_ev = calendar.get()) != NULL )
  {
    ...
    switch(curr_ev->type)
    {
      ...
      case EV_REQ:
        ...
        // Выбор сервера-обработчика
        if (server_0.get_cpu_state() == IDLE){
      ...
      case EV_FIN_0:
        ...
        cout << "Работа CPU_0 с " << run_begin << " по " << curr_time << " длит. " << (curr_time-run_begin) << endl; 
        if (!queue.empty()) 
        {
          Request *rq = queue.front(); 
          queue.pop_front(); 
          calendar.put(new Event(curr_time+rq->time, EV_FIN_0, rq->source_num)); 
          delete rq; 
          run_begin = curr_time;
        }
        break;
      case EV_FIN_1:
        server_1.drop_task();
        // выводим запись о рабочем интервале
        cout << "Работа CPU_1 с " << run_begin << " по " << curr_time << " длит. " << (curr_time-run_begin) << endl; 
        if (!queue.empty()) 
        {
          Request *rq = queue.front(); 
          queue.pop_front(); 
          calendar.put(new Event(curr_time+rq->time, EV_FIN_1, rq->source_num)); 
          delete rq; 
          run_begin = curr_time;
        }
        break;
